...
func (h *HTTPResponseHandler) AssetResponse(content []byte, contentType string) {
	h.rw.Header().Set("Content-Type", contentType)
	h.rw.WriteHeader(http.StatusOK)
	if _, err := h.rw.Write(content); err != nil {
		h.log.Error("write asset HTTP response", core_logger.Error(err))
	}
}
