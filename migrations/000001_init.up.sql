...
CREATE TABLE todoapp.users (
    ...
    user_full_name    VARCHAR(100)  NOT NULL CHECK (
        char_length(user_full_name) BETWEEN 3 AND 100
    ),
    user_phone_number VARCHAR(15) CHECK (
        user_phone_number ~ '^\+[0-9]+$'
        AND char_length(user_phone_number) BETWEEN 10 AND 15
    )
);
...
CREATE TABLE todoapp.tasks (
    ...
    task_title        VARCHAR(100)   NOT NULL CHECK (
        char_length(task_title) BETWEEN 1 AND 100
    ),
    task_description  VARCHAR(1000) CHECK (
        char_length(task_description) BETWEEN 1 AND 1000
    ),
